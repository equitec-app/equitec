module VisitsHelper
  ...
  def max_number_of_visits_per_day(search_employee, date_today, week)
    ...
    7.times do |i|
      ...
      visits_per_day = search_employee.visits.where('visit_date_time BETWEEN ? AND ?', day.beginning_of_day, day.end_of_day ).size
  ...
  def employee_scheduler_per_day(search_employee, day, i)
    visits_of_the_day = search_employee.visits.where('visit_date_time BETWEEN ? AND ?', day.beginning_of_day, day.end_of_day )

    return visits_of_the_day[i].nil? ? "Libre" : visits_of_the_day[i].visit_date_time
